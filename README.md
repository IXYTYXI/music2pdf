...
工作台提供“试听音符（原始时间）”和“试听乐谱节奏”，并可分别导出原始时间 MIDI 与量化后的乐谱 MIDI。原始时间模式保留当前音符的起止时间，不受乐谱 BPM 调整影响；两者均为合成试听，不是原录音。

MusicXML将同谱表、同起止时间的不同音高组成和弦，并省略空闲附加声部的重复休止符。该修复保留识别事件，不代表自动音高、节拍或左右手识别已通过验收。见[修复与验证记录](docs/superpowers/plans/2026-09-10-score-export-repair.md)。
