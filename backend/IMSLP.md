...
2026-09-08 并发同步修复：采集器原子更新元数据期间，旧版同步将 Source changed 计作失败，可能连续三轮后退出。现在该情况单独记录为 deferred，在下一轮重新读取，不增加失败轮数；真正的远端校验错误仍保留为失败。单次同步有 deferred 时返回2，持续模式继续运行。97项后端测试通过，包括更新中断后的再同步和远端错误未被掩盖的回归测试。
