...
p=Path('.work/low-octave-audit');torch.set_num_threads(4)
...
x=sosfiltfilt(butter(4,180,fs=16000,btype="highpass",output="sos"),x).astype("float32")
...
(p/'run.json').write_text(json.dumps({'source_sha256':hashlib.sha256(src.read_bytes()).hexdigest(),'duration':len(x)/16000,'seconds':time.monotonic()-start,'events':len(r['est_note_events']),'score_input':False,'diagnostic_highpass_hz':180,'production_change':False},indent=2))
