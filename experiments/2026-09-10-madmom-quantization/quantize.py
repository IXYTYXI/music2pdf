...
for label,variable,interval,subdivision in [('fixed120',False,.5,4),('fixed94',False,median,4),('dynamic16',True,median,4),('dynamic8',True,median,2)]:
 ...
 for n in notes:
  ...
  transformed.append({**n,'start':start,'duration':end-start})
 ...
 summary.append({'case':label,'events':len(notes),'onset_median_shift_ms':float(np.median(abs(np.array(deltas)))*1000),'onset_p95_shift_ms':float(np.percentile(abs(np.array(deltas)),95)*1000),'onsets_moved_over_80ms':int(sum(abs(np.array(deltas))>.08)),'duration_p95_change_ms':float(np.percentile(abs(np.array(duration_deltas)),95)*1000),'same_pitch_onset_collisions':collapsed,'limit':'Grid origin is first detected beat only, NOT an inferred first downbeat. Meter/phase not selected by this comparison.'})
(p/'cases.json').write_text(json.dumps(cases,indent=2));(p/'quantization.json').write_text(json.dumps(summary,indent=2))
...
(p/'onset-checks.json').write_text(json.dumps(checks,indent=2));print(json.dumps(summary,indent=2));print(json.dumps(checks,indent=2))
