...
const directory=process.argv[2] || '.work/madmom-quantization';
const cases: Record<string,Note[]> = JSON.parse(fs.readFileSync(`${directory}/cases.json`,'utf8'));
const rows=[];
for (const [name,notes] of Object.entries(cases)) {
  for (let phase=0;phase<3;phase++) {
    ...
    fs.writeFileSync(`${directory}/${name}-phase${phase}.musicxml`,xml);
...
fs.writeFileSync(`${directory}/notation-metrics.json`,JSON.stringify(rows,null,2));console.log(rows);
